...
// Property: When SHA-2 mode is selected (hmac_en=0), key processing should be bypassed
property p_sha2_mode_key_bypass;
  @(posedge clk_i) disable iff (!rst_ni)
    (!hmac_en && sha_en) |-> !hmac_fifo_wsel;
endproperty
assert_sha2_mode_key_bypass: assert property(p_sha2_mode_key_bypass);

// Property: When HMAC mode is selected (hmac_en=1), two-round processing should be enabled
property p_hmac_mode_two_rounds;
  @(posedge clk_i) disable iff (!rst_ni)
    (hmac_en && sha_en && hash_start) |-> ##[1:$] (sha_hash_process && $past(sha_hash_done));
endproperty
assert_hmac_mode_two_rounds: assert property(p_hmac_mode_two_rounds);

// Property: Verify digest size configuration properly reflects block size selection
property p_digest_size_block_config;
  @(posedge clk_i) disable iff (!rst_ni)
    (hash_start) |-> 
    ((digest_size == SHA2_256) |-> message_length <= 64'h200) and // 512 bits
    ((digest_size inside {SHA2_384, SHA2_512}) |-> message_length <= 64'h400); // 1024 bits
endproperty
assert_digest_size_block_config: assert property(p_digest_size_block_config);

// Property: Key length should not exceed block size based on digest size
property p_key_length_validation;
  @(posedge clk_i) disable iff (!rst_ni)
    (hmac_en && hash_start) |-> 
    ((digest_size == SHA2_256) |-> (key_length inside {Key_128, Key_256, Key_384, Key_512})) and
    ((digest_size inside {SHA2_384, SHA2_512}) |-> key_length != Key_None);
endproperty
assert_key_length_validation: assert property(p_key_length_validation);

// Helper logic for endian check
logic [31:0] expected_endian_data;
assign expected_endian_data = endian_swap ? {msg_fifo_wdata[7:0], msg_fifo_wdata[15:8], 
                                           msg_fifo_wdata[23:16], msg_fifo_wdata[31:24]} : 
                                           msg_fifo_wdata;

// Property: Verify endian swap configuration affects input message ordering
property p_endian_control_msg;
  @(posedge clk_i) disable iff (!rst_ni)
    (msg_fifo_req && msg_fifo_we) |-> 
    msg_fifo_wdata_endian == expected_endian_data;
endproperty
assert_endian_control_msg: assert property(p_endian_control_msg);

// Property: FIFO full status asserted when all entries used
property p_fifo_full_status;
  @(posedge clk_i) disable iff (!rst_ni)
    (fifo_depth == 6'd32) |-> fifo_full;
endproperty
assert_fifo_full_status: assert property(p_fifo_full_status);

// Property: FIFO full deasserted when entry available
property p_fifo_full_deassert;
  @(posedge clk_i) disable iff (!rst_ni)
    (fifo_full && fifo_rready && fifo_rvalid) |=> !fifo_full;
endproperty
assert_fifo_full_deassert: assert property(p_fifo_full_deassert);

// LLM


// Assumptions about valid configurations
property valid_config_stable_p;
    @(posedge clk_i) disable iff (!rst_ni)
    !invalid_config |=> $stable(!invalid_config);
endproperty
VALID_CONFIG_STABLE: assume property(valid_config_stable_p);

// Main assertions for digest_size validation
property digest_size_reset_p;
    @(posedge clk_i) disable iff (!rst_ni)
    (!rst_ni) |-> (digest_size == SHA2_None);
endproperty
DIGEST_SIZE_RESET: assert property(digest_size_reset_p);

property digest_size_invalid_config_p;
    @(posedge clk_i) disable iff (!rst_ni)
    (invalid_config) |-> (digest_size == SHA2_None);
endproperty
DIGEST_SIZE_INVALID_CONFIG: assert property(digest_size_invalid_config_p);

property digest_size_valid_transition_p;
    @(posedge clk_i) disable iff (!rst_ni)
    (!invalid_config && digest_size != SHA2_None) |-> 
    (digest_size inside {SHA2_256, SHA2_384, SHA2_512});
endproperty
DIGEST_SIZE_VALID_TRANSITION: assert property(digest_size_valid_transition_p);

property digest_size_transition_through_none_p;
    @(posedge clk_i) disable iff (!rst_ni)
    (digest_size != SHA2_None && $past(digest_size) != SHA2_None) |=> 
    (digest_size == $past(digest_size));
endproperty
DIGEST_SIZE_TRANSITION_THROUGH_NONE: assert property(digest_size_transition_through_none_p);


// Property: Verify key_length is set to Key_None after reset
property key_length_reset_p;
    @(posedge clk_i) disable iff (!rst_ni)
    (!rst_ni) |-> (key_length == Key_None);
endproperty
KEY_LENGTH_RESET: assert property(key_length_reset_p);

// Property: Verify key_length defaults to Key_None when configuration is invalid
property key_length_invalid_config_p;
    @(posedge clk_i) disable iff (!rst_ni)
    (invalid_config) |-> (key_length == Key_None);
endproperty
KEY_LENGTH_INVALID_CONFIG: assert property(key_length_invalid_config_p);

// Property: Verify key_length only changes on valid configuration or reset
property key_length_stable_p;
    @(posedge clk_i) disable iff (!rst_ni)
    (key_length != Key_None) && (invalid_config) |=> 
    (key_length == $past(key_length));
endproperty
KEY_LENGTH_STABLE: assert property(key_length_stable_p);

// Property: Verify key_length transitions are valid
property key_length_valid_transition_p;
    @(posedge clk_i) disable iff (!rst_ni)
    (!invalid_config && key_length != Key_None) |-> 
    (key_length inside {Key_128, Key_256, Key_384, Key_512, Key_1024});
endproperty
KEY_LENGTH_VALID_TRANSITION: assert property(key_length_valid_transition_p);


// Property: Verify index is 0 after reset
property index_reset_value_p;
    @(posedge clk_i) disable iff (!rst_ni)
    (!rst_ni) |-> (index == 1'b0);
endproperty
INDEX_RESET_VALUE: assert property(index_reset_value_p);

// Property: Verify index only takes valid values (0 or 1)
property index_valid_range_p;
    @(posedge clk_i) disable iff (!rst_ni)
    (1'b1) |-> (index inside {1'b0, 1'b1});
endproperty
INDEX_VALID_RANGE: assert property(index_valid_range_p);

// Property: Verify index remains 0 for non-SHA384/512 modes
property index_non_sha384_512_p;
    @(posedge clk_i) disable iff (!rst_ni)
    ((digest_size != SHA2_384) && (digest_size != SHA2_512)) |-> (index == 1'b0);
endproperty
INDEX_NON_SHA384_512: assert property(index_non_sha384_512_p);


// Property: Verify err_code is set to NoError after reset
property err_code_reset_value_p;
    @(posedge clk_i) disable iff (!rst_ni)
    (!rst_ni) |-> (err_code == NoError);
endproperty
ERR_CODE_RESET_VALUE: assert property(err_code_reset_value_p);


// Property: Verify err_code transitions through valid values
property err_code_valid_values_p;
    @(posedge clk_i) disable iff (!rst_ni)
    (1'b1) |-> (err_code inside {NoError, SwPushMsgWhenDisallowed, SwUpdateSecretKeyInProcess});
endproperty
ERR_CODE_VALID_VALUES: assert property(err_code_valid_values_p);


// Property: Verify idle_q takes MuBi4False value immediately after reset
property idle_q_reset_value_p;
    @(posedge clk_i) disable iff (0)
    (!rst_ni) |-> (idle_q == prim_mubi_pkg::MuBi4False);
endproperty
IDLE_Q_RESET_VALUE: assert property(idle_q_reset_value_p);

// Property: Verify idle_q captures idle_d value in the next clock cycle when not in reset
property idle_q_update_p;
    @(posedge clk_i) disable iff (!rst_ni)
    (1'b1) |=> (idle_q == $past(idle_d));
endproperty
IDLE_Q_UPDATE: assert property(idle_q_update_p);

// Property: Verify idle_q maintains its value when idle_d is stable  
property idle_q_stable_value_p;
    @(posedge clk_i) disable iff (!rst_ni)
    ($stable(idle_d)) |=> ($stable(idle_q));
endproperty
IDLE_Q_STABLE_VALUE: assert property(idle_q_stable_value_p);

// Property: Verify idle_q is not affected by idle_d changes during reset
property idle_q_reset_immunity_p;
    @(posedge clk_i) disable iff (0)
    (!rst_ni && $changed(idle_d)) |-> (idle_q == prim_mubi_pkg::MuBi4False);
endproperty
IDLE_Q_RESET_IMMUNITY: assert property(idle_q_reset_immunity_p);

// Property: Verify idle_q only takes valid MuBi4 values
property idle_q_valid_values_p;
    @(posedge clk_i) disable iff (!rst_ni)
    (1'b1) |-> (idle_q inside {prim_mubi_pkg::MuBi4True, prim_mubi_pkg::MuBi4False});
endproperty
IDLE_Q_VALID_VALUES: assert property(idle_q_valid_values_p);
